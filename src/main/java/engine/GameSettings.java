...
public class GameSettings {
    ...
    public static List<StageData> parseStages(Path path) throws Exception {
        ...
        for (String block : stageBlocks) {
            List<String> lines = Arrays.stream(block.split("\n"))
                .map(String::trim)
                .filter(
                    s -> !s.isEmpty() && !s.startsWith("//")) // Deleting comments and empty lines
                .toList();
            if (lines.isEmpty()) {
                continue;
            }
    ...
    public static List<GameSettings> getGameSettings() {
        ...
        // res/ 경로 변경
        try {
            String resourcePath = "game_data/level.csv";
            URL resourceUrl = GameSettings.class.getClassLoader().getResource(resourcePath);
            if (resourceUrl == null) {
                throw new java.io.FileNotFoundException("Resource not found: " + resourcePath);
            }
            Path resourcePathObject = Paths.get(resourceUrl.toURI());
            stageDataList = parseStages(resourcePathObject);
