...
public class ShipSelectionMenuRenderer {
    ...
    public void drawShipSelectionMenu(Graphics g, final Screen screen, final Ship[] shipExamples,
        final int selectedShipIndex, final int playerIndex) {
        ...
        String[] shipFireRates = {"FIRE RATE: NORMAL", "FIRE RATE: NORMAL", "FIRE RATE: NORMAL",
            "FIRE RATE: SLOW"};
        
        entityRenderer.drawEntity(g, ship, ship.getPositionX() - ship.getWidth() / 2,
            ship.getPositionY());
//        for (int i = 0; i < 4; i++) {
//            // Draw Player Ship
...
//        }
        ...
        commonRenderer.drawCenteredRegularString(g, screen,
            " > " + shipNames[selectedShipIndex] + " < ", screen.getHeight() / 2 - 40);
...
//        drawCenteredRegularString(shipFireRates[selectedShipIndex], centerX, screen.getHeight() / 2 + 80);
        commonRenderer.drawCenteredRegularString(g, screen, shipSpeeds[selectedShipIndex],
            screen.getHeight() / 2 + 60);
        commonRenderer.drawCenteredRegularString(g, screen, shipFireRates[selectedShipIndex],
