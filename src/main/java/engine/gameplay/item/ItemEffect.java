...
public class ItemEffect {
     ...
     * <p>
     * NOTE: This helper uses the existing GameState API (getCoins and addCoins). It subtracts coins
     * by calling addCoins(playerIndex, -cost). If your GameState provides a dedicated "spend" or
     * "removeCoins" method you can replace the call.
     */
    private static boolean trySpendCoins(final GameState gameState, final int playerId,
        final int cost) {
        if (gameState == null) {
            return false;
        }
        ...
        if (gameState.spendCoins(playerIndex, cost)) {
            logger.info(
                "Player " + playerId + " spent " + cost + " coins. before: " + current + ", after: "
                    + gameState.getCoins());
            return true;
        } else {
            logger.info("Player " + playerId + " cannot afford cost " + cost + ". current coins: "
                + current);
            return false;
        }
    }
