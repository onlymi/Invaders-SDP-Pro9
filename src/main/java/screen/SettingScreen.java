...
public class SettingScreen extends Screen {
    ...
    protected final void update() {
        ...
        // Change key settings
        else if (this.selectMenuItem == firstplayerMenu
            || this.selectMenuItem == secondplayerMenu) {
            if (inputManager.isKeyDown(KeyEvent.VK_RIGHT) && this.inputCooldown.checkFinished()
                && !waitingForNewKey && selectedSection == 0) {
                this.selectedSection = 1;
                this.selectedKeyIndex = 0;
                this.inputCooldown.reset();
            }
            if (this.selectedSection == 1 && inputManager.isKeyDown(KeyEvent.VK_LEFT)
                && this.inputCooldown.checkFinished() && !waitingForNewKey) {
                selectedSection = 0;
                this.inputCooldown.reset();
            }
            if (this.selectedSection == 1 && inputManager.isKeyDown(KeyEvent.VK_UP)
                && this.inputCooldown.checkFinished() && selectedKeyIndex > 0
                && !waitingForNewKey) {
                selectedKeyIndex--;
                this.inputCooldown.reset();
            }
            if (this.selectedSection == 1 && inputManager.isKeyDown(KeyEvent.VK_DOWN)
                && this.inputCooldown.checkFinished() && selectedKeyIndex < keyItems.length - 1
                && !waitingForNewKey) {
                selectedKeyIndex++;
                this.inputCooldown.reset();
            }
            // Start waiting for new keystrokes
            if (this.selectedSection == 1 && inputManager.isKeyDown(KeyEvent.VK_SPACE)
                && this.inputCooldown.checkFinished() && !waitingForNewKey) {
            ...
            if (waitingForNewKey) {
                ...
                if (newKey != -1 && this.inputCooldown.checkFinished()) {
                    ...
                    if (newKey == KeyEvent.VK_ESCAPE || newKey == KeyEvent.VK_BACK_SPACE) {
                        System.out.println(
                            "Key setting change cancelled : " + KeyEvent.getKeyText(newKey)
                                + " input");
                    ...
                    int[] targetKeys =
                        (this.selectMenuItem == firstplayerMenu) ? player1Keys : player2Keys;
                    int[] otherKeys =
                        (this.selectMenuItem == firstplayerMenu) ? player2Keys : player1Keys;
    ...
    private void draw() {
        ...
        drawManager.getSettingScreenRenderer()
            .drawSettingMenu(drawManager.getBackBufferGraphics(), this);
        drawManager.getSettingScreenRenderer()
            .drawSettingLayout(drawManager.getBackBufferGraphics(), this, this.menuItem,
                this.selectMenuItem);
        
        switch (this.selectMenuItem) {
            case volumeMenu:
                drawManager.getSettingScreenRenderer()
                    .drawVolumeBar(drawManager.getBackBufferGraphics(), this, this.volumelevel,
                        this.draggingVolume);
                break;
            case firstplayerMenu:
                drawManager.getSettingScreenRenderer()
                    .drawKeySettings(drawManager.getBackBufferGraphics(), this, 1,
                        this.selectedSection, this.selectedKeyIndex, this.keySelected,
                        this.player1Keys);
                break;
            case secondplayerMenu:
                drawManager.getSettingScreenRenderer()
                    .drawKeySettings(drawManager.getBackBufferGraphics(), this, 2,
                        this.selectedSection, this.selectedKeyIndex, this.keySelected,
                        this.player2Keys);
                break;
        }
        ...
        int mx = inputManager.getMouseX();
        int my = inputManager.getMouseY();
        java.awt.Rectangle backBox = Core.getHitboxManager()
            .getBackButtonHitbox(drawManager.getBackBufferGraphics(), this);
