...
public class ShipSelectionScreen extends Screen {
    ...
    public ShipSelectionScreen(final int width, final int height, final int fps, final int player) {
        ...
        shipType = new Ship.ShipType[]{Ship.ShipType.NORMAL, Ship.ShipType.BIG_SHOT,
            Ship.ShipType.DOUBLE_SHOT, Ship.ShipType.MOVE_FAST};
        ...
        for (int i = 0; i < shipExamples.length; i++) {
            ...
            if (player == 1) {
                shipExamples[i] = new Ship(positionX, height / 2, Entity.Team.PLAYER1, shipType[i],
                    null);
            } else if (player == 2) {
                shipExamples[i] = new Ship(positionX, height / 2, Entity.Team.PLAYER2, shipType[i],
                    null);
            }
        }
    }
    ...
    private void draw() {
        ...
        drawManager.getShipSelectionMenuRenderer()
            .drawShipSelectionMenu(drawManager.getBackBufferGraphics(), this, shipExamples,
                this.selectedShipIndex, this.player);
